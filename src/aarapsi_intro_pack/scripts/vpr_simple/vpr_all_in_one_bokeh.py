...
from aarapsi_intro_pack import VPRImageProcessor, Tolerance_Mode, FeatureType, \
                               labelImage, makeImage, \
                               doMtrxFigBokeh, doDVecFigBokeh, doOdomFigBokeh, \
                               updateMtrxFigBokeh, updateDVecFigBokeh, updateOdomFigBokeh 
...
class mrc: # main ROS class
    def __init__(self, database_path, ref_images_path, ref_odometry_path, image_feed_input, odometry_input, dataset_name, doc_handle, \
        ...
        if self.DO_PLOTTING:
            ...
            self.fig_mtrx_handles = doMtrxFigBokeh(self, self.ref_dict['odom']) # Make simularity matrix figure
            ...
            self.DOC.add_root(row(self.fig_mtrx_handles['fig'], self.fig_dvec_handles['fig'], self.fig_odom_handles['fig']))
...
# Main loop process
    ...
    if nmrc.DO_PLOTTING: # set by node input
        ...
        updateMtrxFigBokeh(nmrc, matchInd, trueInd, dvc, nmrc.ref_dict['odom'])
...
def main(doc):
    ...
    nmrc = mrc(args['database-path'], args['ref-imgs-path'], args['ref-odom-path'], args['image-topic-in'], args['odometry-topic-in'], \
                args['dataset-name'], doc, compress_in=args['compress_in'], compress_out=args['compress_out'], do_plotting=args['do_plotting'], do_image=args['make_images'], \
    ...
    root = rospkg.RosPack().get_path(rospkg.get_package_name(os.path.abspath(__file__))) + "/scripts/testing/"
