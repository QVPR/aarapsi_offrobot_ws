...
ft_raw = ip.SET_DICT['img_feats']['RAW']['forward'][ind]
img = np.dstack((np.reshape(ft_raw, dims),)*3).astype(np.uint8)
...
netvlad_obj = NetVLAD_Container(cuda=gpu_exists, ngpus=int(gpu_exists), logger=lambda x: (x))
...
# t1 = rospy.Time.now().to_sec()
# ref_ftrs = aarapsi_NetVLAD_feature_extract.feature_ref_extract(ref_path)
# t2 = rospy.Time.now().to_sec()
# rospy.loginfo("Duration: %s" % (str(t2-t1)))
